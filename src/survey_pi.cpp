...
survey_pi::survey_pi(void* ppimgr) : opencpn_plugin_118(ppimgr) {
  ...
  if (icon_path.type == IconPath::Type::Svg)
    ...
    wxLogWarning("Cannot find icon for basename: %s", "survey_panel_icon");
  ...
  while (parameter) {
    ...
    if (paramname == _T("name")) {
      ...
      wxProgressDialog progressdialog(
          _("Import Hydromagic file"), _("Adding soundings"), ct,
          m_pSurveyDialog, wxPD_CAN_ABORT | wxPD_APP_MODAL);
...
void survey_pi::ExportHydromagic(int survey_id, wxString filename) {
  ...
  wxString sql = wxString::Format(
      _T("SELECT depth, AsText(geom), tide, measured ")
...
bool survey_pi::ImportGPX(wxString filename) {
  ...
  wxProgressDialog progressdialog(_("Import Garmin GPX"), _("Adding soundings"),
                                  ct, m_pSurveyDialog,
                                  wxPD_CAN_ABORT | wxPD_APP_MODAL);
  ...
  for (std::vector<GPXWayPoint>::iterator it = m_pSurveyDialog->trkpts.begin();
...
double survey_pi::dbGetHeightOfTide(wxDateTime depthTime, int surv) {
  wxString sql = wxString::Format(
      _T("SELECT measured, height FROM tide_height WHERE survey_id=%i"), surv);
  char** results;
  int n_rows;
  int n_columns;
  char* dpt;
...
bool survey_pi::dbSaveTideHeights(int survey1) {
  ...
  n_rows =
      m_pSurveyDialog->m_pSurveyTidalDialog->m_gdTidalHeights->GetNumberRows();
  ...
  for (i = 0; i < n_rows; i++) {
    tidetime =
        m_pSurveyDialog->m_pSurveyTidalDialog->m_gdTidalHeights->GetCellValue(
            i, 0);
...
void survey_pi::dbGetTable(wxString sql, char*** results, int& n_rows,
                           int& n_columns) {
  ret = sqlite3_get_table(m_database, sql.mb_str(), results, &n_rows,
                          &n_columns, &err_msg);
  if (ret != SQLITE_OK) {
...
wxArrayString survey_pi::SetSoundings(int i) {
  ...
  // wxString myInt = wxString::Format(wxT("%i"),i);
  wxString myString =
      wxString::Format(_T("SELECT * FROM sounding WHERE survey_id = %i"), i);
  dbGetTable(myString, &result, n_rows, n_columns);
  wxArrayString soundings;
  for (int i = 1; i <= n_rows; i++) {
    char* id = result[(i * n_columns) + 0];
...
int survey_pi::CreateSurvey(wxString name) {
  wxString sql = wxString::Format(
      _T("INSERT INTO survey (survey_name, created, ")
...
void survey_pi::DeleteSurvey(int id) {
  ...
  if (c != 0) {
    ...
    if (testEdit == _T("Edt.")) {
      ...
      wxString sql =
          wxString::Format(_T("DELETE FROM sounding WHERE survey_id=%i"), id);
      dbQuery(sql);
      sql = wxString::Format(_T("DELETE FROM survey WHERE survey_id=%i"), id);
      dbQuery(sql);
    }
...
void survey_pi::DeleteSounding(int id) {
  ...
  if (m_activesurvey != 0) {
    ...
    if (s.length() > 4) {
      ...
      wxString sql =
          wxString::Format(_T("DELETE FROM sounding WHERE sounding_id=%i"), id);
      dbQuery(sql);
    }
  }
}
...
int survey_pi::InsertSounding(double depth, double lat, double lon, double tide,
  ...
  if (t == -1) {
    wxMessageBox(
        _("No survey selected, please select or create/select a new survey"));
    return 0;
  }
  wxString s = m_pSurveyDialog->m_chSurvey->GetStringSelection();
  int i = GetSurveyId(s);
...
int survey_pi::Init(void) {
  ...
  if (newDB && b_dbUsable) {
    ...
    dbQuery(sql);
    dbQuery(
        _T("SELECT AddGeometryColumn('sounding', 'geom', 32632, ")
        _T("'POINT', 2)"));
  }
  ...
  if (b_dbUsable) {
    ...
    if (ret != SQLITE_OK) {
      sqlite3_free(err_msg);
      sql =
          _T("CREATE TABLE settings (")
          _T("key TEXT NOT NULL UNIQUE,")
          _T("value TEXT)");
      dbQuery(sql);
      dbQuery(
          _T("INSERT INTO settings (key, value) VALUES ")
          _T("('DBVersion', '2')"));
      db_ver = 2;
    } else {
  ...
  if (b_dbUsable && db_ver == 3) {
    ...
    dbQuery(sql);
    dbQuery(
        _T("SELECT AddGeometryColumn('new_sounding', 'geom', 3395, ")
        _T("'POINT', 2)"));
    dbQuery(
    ...
    dbQuery(sql);
    dbQuery(
        _T("SELECT AddGeometryColumn('sounding', 'geom', 3395, ")
        _T("'POINT', 2)"));
    dbQuery(
        _T("INSERT INTO sounding SELECT sounding_id, depth, measured, ")
  ...
  if (b_dbUsable && db_ver == 4 && newDB) {
    sql =
        _T("CREATE TABLE edited_sounding (")
        _T("sounding_id INTEGER PRIMARY KEY AUTOINCREMENT,")
        _T("depth REAL NOT NULL,")
        _T("measured INTEGER NOT NULL,")
        _T("survey_id INTEGER NOT NULL,")
    ...
    dbQuery(sql);
    dbQuery(
        _T("SELECT AddGeometryColumn('edited_sounding', 'geom', 3395, ")
        _T("'POINT', 2)"));

    sql =
        _T("CREATE TABLE tide_height (")
        _T("measured INTEGER NOT NULL,")
  ...
  if (m_bSurveyShowIcon)
    ...
    m_leftclick_tool_id = InsertPlugInToolSVG(
        _T("Survey"), _svg_survey, _svg_survey, _svg_survey_toggled,
        wxITEM_CHECK, _("Survey"), _T(""), NULL, SURVEY_TOOL_POSITION, 0, this);
#else
    m_leftclick_tool_id = InsertPlugInTool(_T(""), _img_survey, _img_survey,
                                           wxITEM_CHECK, _("Survey"), _T(""),
...
wxString survey_pi::GetLongDescription() {
  return _(
      "Records depth from the NMEA stream\n\
...
void survey_pi::SetColorScheme(PI_ColorScheme cs) {
  if (NULL == m_pSurveyDialog) return;
...
void survey_pi::ShowPreferencesDialog(wxWindow* parent) {
  SurveyCfgDlg* dialog = new SurveyCfgDlg(
      parent, wxID_ANY, _("Survey Preferences"),
      wxPoint(m_survey_dialog_x, m_survey_dialog_y), wxSize(650, 650),
...
bool survey_pi::GetSurveySoundings(int as) {
  ...
  if (!b_dbUsable || !m_bRenderOverlay) return false;
  ...
  wxString sql = wxString::Format(
      _T("SELECT depth, AsText(geom), sounding_id, ")
...
void survey_pi::StoreSounding(double depth) {
  ...
  if (m_latprev == 0) {
    ...
    m_pSurveyDialog->mySurveyTrace->SetData(OCPN_DBP_STC_MDA, depth,
                                            _("meters"));
  }
  if (!m_latprev == 0) {
    DistanceBearingMercator_Plugin(m_latprev, m_lonprev, m_lat, m_lon, &brg,
                                   &dist);
